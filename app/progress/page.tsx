...
export default function ProgressPage() {
  const uid       = 'demoUser';                      // TODO: 認証と連携
  const todayKey  = format(new Date(), 'yyyyMMdd');
  const todayDisp = format(new Date(), 'M/d（eee）');
  ...
  useEffect(() => {
    ...
    return onSnapshot(col, snap => {
      const t: Record<string, TodoItem> = {};
      snap.forEach(d => (t[d.id] = { id: d.id, ...(d.data() as any) }));
      setTodos(t);
    });
  }, [uid, todayKey]);
  ...
  const cards = useMemo(() => {
    return Object.values(materials).map(mat => {
      ...
      const planEnd   = Math.min(planStart + mat.dailyPlan - 1, mat.totalCount);
  ...
  const handleSave = async (p: {
    id: string;
    doneStart: number | null; doneEnd: number | null;
    prevStart: number | null; prevEnd: number | null;
  }) => {
    await saveProgress({
      uid,
      materialId: p.id,
      newStart: p.doneStart ?? 0,
      newEnd:   p.doneEnd   ?? 0,
      ...
      {cards.map(c => (
        <ProgressCard
          ...
          onSave={({ id, doneStart, doneEnd }) =>
            handleSave({ id, doneStart, doneEnd, prevStart: c.prevStart, prevEnd: c.prevEnd })
          }
        />
      ))}
