    ...
    const handle = async () => {
      try {
        ...
        if (!code || !state) {
          ...
          setTimeout(() => router.push('/login?error=params'), 1400);
          return;
        }

        // CSRF: state 検証（フロントは sessionStorage に統一）
        const expectedState = sessionStorage.getItem('line_state');
        if (state !== expectedState) {
          setStatus('認証状態が一致しません');
          setTimeout(() => router.push('/login?error=state'), 1400);
          return;
        }
        ...
        setStatus('認証に失敗しました');
        setTimeout(() => router.push('/login?error=server'), 1600);
      }
    };
